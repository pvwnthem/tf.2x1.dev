...
export const POST = async (req: Request, res: NextApiResponse) => {
  try {
    ...
    if (title.length > 56) {
        return new NextResponse("Title must be 56 or less characters long", {
            status: 409,
        });
    }

    if (content.length > 2048) {
        return new NextResponse("Content must be 2048 or less characters long", {
            status: 409,
        });
    }
    const postData = {
    ...
    if (error.response) {
      ...
      return new NextResponse("An error occurred while creating the forum post.", {
        status: 500,
    });
    }
  }
